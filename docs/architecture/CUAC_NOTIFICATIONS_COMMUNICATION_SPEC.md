...
Data-rights notifications use a mandatory `privacy_requests` topic for student, CUAC Ops and CUAC Admin audiences. A dedicated scheduler creates the 24-hour/day-five identity reminders and the assigned-reviewer internal/effective-deadline reminders. Its ledger and notification publication commit atomically, use stable event identities and follow an approved extended deadline. This scheduler does not contact a mail provider: email remains a queued delivery for the general notification worker and stays disabled until the sender domain is accepted.
