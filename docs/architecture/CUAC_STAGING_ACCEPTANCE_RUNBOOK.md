...
| `workers.supervision_and_recovery` | Every worker required by the selected release scope is supervised; kill/restart and lease recovery complete without duplicate business effects. For `school-handoff-v1`, supervise Auth-email and notification workers only, and verify payment, file and official-material workers are not deployed. |
