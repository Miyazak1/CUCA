...
Internationalization is now a release workstream rather than an English/Chinese page patch. The canonical UI locale contract covers English, Vietnamese, Thai, Indonesian, Malay and Arabic for the public student experience, while Simplified Chinese remains available for school, operations and existing reviewed policy scopes. Locale normalization, browser preference ordering, account-locale projection and Arabic RTL metadata are implemented. The public home page, shared navigation, site search and the university, program and scholarship catalog discovery surfaces are the released multilingual slices: selection is URL-based, browser-aware and does not create durable browser state. Catalog navigation preserves the selected locale; detailed filters, sorting, card UI status, result context, dynamic loading, failure, empty, count, pagination and save controls share one bounded dictionary. The three core catalog detail pages also share localized navigation, source/readiness state, summary and field labels, primary actions, errors, locale-aware dates and URL-language continuity. Search includes reviewed canonical-term mappings for common Vietnamese, Thai, Indonesian, Malay and Arabic admissions phrases, while non-English surfaces explicitly disclose that official catalog names, admissions content, city pages, guides and long-form guidance remain in their reviewed source language pending translations. Legal and consent content never silently falls back. See `CUAC_I18N_ARCHITECTURE.md` for rollout and per-language acceptance gates.
