    ...
    -> one immutable CUAC Node image
      ...
      -> same-origin /api/v1 boundary
        ...
        -> domain services and transaction/audit layer
          ...
          -> Data-rights reminder worker -> Notification outbox
...
| Main server suite | 703/703 pass, plus 23/23 focused data-rights and 27/27 notification tests | The complete server suite and focused student/Ops rights and notification tests, including password-step-up identity evidence, database-clock deadline classification, bounded step-up deadline extension, bilingual fixed-copy transition notices and automatic reminder scheduling, the simplified approval matrix and thin non-execution route boundary, detached reproducible migration package, Auth-email invitation templates, legal policy schemas and tamper rejection, are green. |
...
For `school-handoff-v1`, Auth-email, notification and data-rights reminder workers are in scope. The reminder worker creates durable notification deliveries only; the notification worker performs any later external delivery. Payment, file and official-material workers must be absent/disabled and their acceptance controls recorded as `not_applicable`.
