...
| Main server suite | 702/702 pass | The complete server suite, including student and Ops data-rights policy boundaries, detached reproducible migration package, Auth-email invitation templates, legal policy schemas and tamper rejection, is green. |
| Real PostgreSQL rehearsal | 422 tests pass | PostgreSQL 16.15 applied and replayed the complete migration chain, matched 84 tables/1,276 columns/486 constraints/322 indexes, exercised student ownership plus grant-bound Ops claim/escalation, historical upgrades, concurrency and authorization, and removed the disposable database afterward. |
| Migration release | Pass | The detached release was reproduced as `5e6311a6e7a34e4d1f2da3b184aca75ec2c9d56325389e7c71875e1c3329a045` with 15 pinned runtime dependencies. |
...
Implementation status: **stable fail-closed policy routes, governed publication scopes, authenticated student intake and grant-bound internal triage are implemented; approved wording and operational fulfilment remain open release blockers.**
...
The account preferences area now accepts structured access, correction, portable-export and account-deletion requests, lists only the signed-in student's records and permits revision-safe cancellation while a request is still received. Export and deletion require fresh password verification. The PostgreSQL boundary rechecks the active account and student role, permits only one active request per type, preserves a minimum one-way subject reference after account deletion and couples create/cancel writes to metadata-only audit. Eleven student/Ops service and HTTP tests, five preferences contracts, the production build, lint, migration-history checks and the disposable PostgreSQL migration rehearsal are green. This is controlled intake and triage only: no export artifact, erasure job, SLA outcome, notification or legal decision is implied.

The Ops console now has a separate minimal triage queue. Claim and escalation operations are optimistic-lock protected, transactionally audited and bound to the operator's current approved CUAC staff grant. The queue never exposes account email, profile, education, files or application content. It intentionally offers no completion, denial, export or deletion command; those remain blocked until the legal scope, identity-verification method, retention exceptions, SLA and dual-control runbook are approved.
