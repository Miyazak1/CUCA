...
export type OpsAccountDeletionHttpService = Pick<OpsAccountDeletionService, "list" | "refresh" | "reviewLegalHold" | "quarantine">;
export function createOpsAccountDeletionHttpHandlers(service: OpsAccountDeletionHttpService, auth: AuthSessionRepository) {
  ...
  return {
    ...
    quarantine: (request: Request, id: string) => run(request, async context => service.quarantine(context, id,
      inputRecord(await request.json(), ["quarantineId", "expectedRevision"]))),
  };
}
