...
test("catalog save actions use authenticated student saved-item APIs", async () => {
  ...
  assert.match(universities, /save catalog-save-control/);
  assert.match(scholarships, /save-button catalog-save-control/);
  assert.match(programs, /CuacCatalogList\.saveHeartIcon\(\)/);
  assert.match(universities, /CuacCatalogList\.saveHeartIcon\(\)/);
  assert.match(scholarships, /CuacCatalogList\.saveHeartIcon\(\)/);
...
test("scholarship pagination stays compact and exposes the current page", async () => {
  ...
  assert.match(css, /\.scholarship-card-open\s*\{[^}]*width:\s*34px[^}]*height:\s*34px[^}]*color:\s*var\(--accent\)/s);
  assert.match(css, /\.scholarship-card-open svg\s*\{[^}]*width:\s*17px[^}]*fill:\s*none[^}]*stroke:\s*currentColor[^}]*stroke-linecap:\s*round/s);
});
