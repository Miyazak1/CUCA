...
function fixture() { const calls = [], service = Object.fromEntries(["list", "refresh", "reviewLegalHold"].map(method =>
...
test("account-deletion HTTP binds purpose, route id and strict command bodies", async () => {
  ...
  assert.deepEqual(calls.map(call => call.method), ["list", "refresh", "reviewLegalHold"]);
