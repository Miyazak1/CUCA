...
const outputPath = resolve(root, "seeds/catalog.beihang-school-program-batch-01.approved.local.json");
const approvalPath = resolve(root, "seeds/catalog.beihang-school-program-batch-01.approval.json");
...
if (actualReviewHash !== approvedReviewHash) throw new Error("User-approved review hash does not match the stored Beihang University review.");
...
if (!freshValidation.ok || freshValidation.bundleSha256 !== approvedCandidateHash) throw new Error("Stored Beihang review does not match a valid candidate bundle.");
...
if (review.status !== "awaiting_user_approval" || review.candidateBundleSha256 !== approvedCandidateHash) throw new Error("Beihang review is not awaiting approval for this candidate.");
if (review.sensitiveDataCheck?.result !== "pass" || review.sensitiveDataCheck?.scope !== "candidate bundle only") throw new Error("Beihang candidate has not passed prohibited-data review.");
if (
  review.scope?.schoolSlug !== "beihang-university" || review.scope?.schoolCount !== 1 ||
  review.scope?.programCount !== 76 || review.scope?.intakeCount !== 76 ||
  review.scope?.chineseProgramCount !== 57 || review.scope?.englishProgramCount !== 11 || review.scope?.frenchProgramCount !== 8 ||
  review.scope?.beijingProgramCount !== 72 || review.scope?.hangzhouProgramCount !== 4
) throw new Error("Beihang review scope must contain exactly one school, 76 language/campus-qualified routes, and 76 intakes.");
...
if (!Array.isArray(legacyAliases) || legacyAliases.length !== 1 || legacyAliases[0] !== "beihang-university-electronic-information-engineering-english") throw new Error("Beihang reconciliation must archive only the approved conflicting English Electronic Information Engineering record.");
if (!Array.isArray(stableMatchingSlugs) || stableMatchingSlugs.length !== 17 || new Set(stableMatchingSlugs).size !== 17) throw new Error("Beihang reconciliation must contain seventeen distinct stable matches.");
if ((candidate.programs ?? []).some(program => legacyAliases.includes(program.slug))) throw new Error("The archive record overlaps the approved current catalog.");
...
const publication: CatalogSeedBundle = {
  ...
  handoff: {
    sourceSystem: "CUAC reviewed official Beihang University 2026 admissions, program catalog, CSCA and campus sources",
    cleanedExportName: "catalog.beihang-school-program-batch-01.draft.json",
...
if (!publicationReport.ok || publicationReport.summary.cities !== 2 || publicationReport.summary.schools !== 1 || publicationReport.summary.programs !== 76 || publicationReport.summary.programIntakes !== 76) throw new Error(`Publication bundle is invalid: ${publicationReport.errors.join(" ")}`);
const approvalRecord = {
  version: 1,
  approvedAt: approvalRecordedAt,
  reviewReference,
  approvedReviewSha256: approvedReviewHash,
  approvedCandidate: "seeds/catalog.beihang-school-program-batch-01.draft.json",
  approvedCandidateBundleSha256: approvedCandidateHash,
  publicationBundle: "seeds/catalog.beihang-school-program-batch-01.approved.local.json",
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:beihang-school-program-publish" });
try {
  ...
  const result = await client.transaction(async tx => {
    ...
    if (schoolRows.length !== 1 || schoolRows[0]?.status !== "active" || schoolRows[0]?.verification_status !== "unverified" || schoolRows[0]?.program_count !== "18") throw new Error("Beihang no longer matches the reviewed active/unverified school with eighteen active programs.");
    const reviewedRows = await tx.query<{ slug: string; status: string; verification_status: string }>(
      ...
      ["beihang-university", [...stableMatchingSlugs, ...legacyAliases]],
    );
    if (reviewedRows.length !== 18 || reviewedRows.some(row => row.status !== "active" || row.verification_status !== "unverified")) throw new Error("The seventeen stable matches and approved conflicting record no longer match the reviewed state.");
    ...
    const archived = await tx.query<{ slug: string }>(
      ...
      ["beihang-university", legacyAliases],
    );
    if (archived.length !== 1 || archived[0]?.slug !== legacyAliases[0]) throw new Error("Approved conflicting-record archival count changed during publication.");
    ...
    if (finalRows[0]?.program_count !== "76" || finalRows[0]?.verified_count !== "76") throw new Error("Post-publication Beihang program counts are not exactly 76 verified active records.");
    return { written, stableMatches: stableMatchingSlugs.slice().sort(), archived: archived.map(row => row.slug).sort(), finalProgramCount: 76 };
  });
