...
const pool = createPostgresPool({ databaseUrl: localDatabaseUrl(state), max: 1, applicationName: "cuac:heu-complete-preflight" });

try {
  const schools = await pool.query(`select id,slug,name_en,name_zh,status,verification_status,source_url,city_id
    from schools where slug='harbin-engineering-university'
      or lower(name_en)=lower('Harbin Engineering University')
      or name_zh='哈尔滨工程大学' order by slug`);
