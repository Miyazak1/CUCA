...
const paths = {
  candidate: resolve(root, "seeds/catalog.sdu-complete-batch-01.draft.json"),
  validation: resolve(root, "seeds/catalog.sdu-complete-batch-01.validation.json"),
  review: resolve(root, "seeds/catalog.sdu-complete-batch-01.review.json"),
  output: resolve(root, "seeds/catalog.sdu-complete-batch-01.approved.local.json"),
  approval: resolve(root, "seeds/catalog.sdu-complete-batch-01.approval.json"),
...
if (storedReviewHash !== approvedReviewHash || actualReviewHash !== approvedReviewHash) throw new Error("Standing-authorized review hash does not match the stored SDU review.");
if (createHash("sha256").update(candidateText).digest("hex") !== review.candidateSha256) throw new Error("SDU candidate changed after review.");
const freshValidation = createCatalogMigrationValidationReport(candidate);
// candidateSha256 above authenticates the exact reviewed file bytes. Compare
// the deterministic operation plan as an additional semantic guard. Older
// validation artifacts may have hashed an in-memory builder containing
// undefined optional properties, which JSON serialization correctly omits.
if (!freshValidation.ok || !storedValidation.ok || freshValidation.operationPlanSha256 !== storedValidation.operationPlanSha256) throw new Error("SDU candidate validation no longer matches the reviewed bundle.");
if (review.status !== "standing_user_approval" || review.standingAuthorization?.instruction !== "发布默认允许" || review.sensitiveDataCheck?.result !== "pass" || review.visualReview?.result !== "pass") throw new Error("SDU review is not eligible for standing-authorized publication.");
if (review.scope?.schoolSlug !== "shandong-university" || review.scope?.schoolCount !== 1 || review.scope?.programRouteCount !== 143 || review.scope?.programIntakeCount !== 143 || review.scope?.scholarshipCount !== 5) throw new Error("SDU review scope mismatch.");
...
const publication: CatalogSeedBundle = {
  ...
  handoff: { sourceSystem: "CUAC reviewed official Shandong University 2026 sources", cleanedExportName: "catalog.sdu-complete-batch-01.draft.json", cleanedExportSha256: freshValidation.bundleSha256, sourceSchemaSha256, reviewReference, prohibitedDataReviewReference: reviewReference, approvalRecordedAt, sourceReadOnly: true, prohibitedDataConfirmedExcluded: true },
...
if (!publicationReport.ok || publicationReport.summary.schools !== 1 || publicationReport.summary.programs !== 143 || publicationReport.summary.programIntakes !== 143 || publicationReport.summary.scholarships !== 5) throw new Error(`SDU publication bundle is invalid: ${publicationReport.errors.join(" ")}`);
const approvalRecord = { version: 1, approvedAt: approvalRecordedAt, reviewReference, approvedReviewSha256: approvedReviewHash, approvalMode: "standing-user-default-publication", approvedCandidate: "seeds/catalog.sdu-complete-batch-01.draft.json", approvedCandidateBundleSha256: freshValidation.bundleSha256, publicationBundle: "seeds/catalog.sdu-complete-batch-01.approved.local.json", publicationBundleSha256: publicationReport.bundleSha256, schoolSlug: "shandong-university", programSlugs: publication.programs?.map(row => row.slug), scholarshipSlugs: publication.scholarships?.map(row => row.slug), archivedProgramAliases: [], archivedScholarshipAliases: [], prohibitedDataConfirmedExcluded: true };
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:sdu-complete-publish" });
try {
  ...
  const result = await client.transaction(async tx => {
    ...
    const before = await tx.query<{ id: string }>("select id from schools where slug=$1 or lower(name_en)=lower($2) or name_zh=$3", ["shandong-university", "Shandong University", "山东大学"]);
    if (before.length !== 0) throw new Error("SDU record now exists; standing authorization requires a new conflict review before overwrite.");
    const city = await tx.query<{ name_en: string; name_zh: string; region: string; province: string | null }>("select name_en,name_zh,region,province from cities where slug=$1", ["jinan"]);
    if (city.length !== 1 || city[0].name_en !== "Jinan" || city[0].name_zh !== "济南" || city[0].region !== "华北" || city[0].province !== null) throw new Error(`Existing Jinan city record is not the reviewed reference target: ${JSON.stringify(city)}`);
