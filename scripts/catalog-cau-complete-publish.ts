...
const paths = {
  ...
  output: resolve(root, "seeds/catalog.cau-complete-batch-01.approved.local.json"),
  approval: resolve(root, "seeds/catalog.cau-complete-batch-01.approval.json"),
...
if (sha(JSON.stringify(reviewBase)) !== reviewHash || publicationReference !== `standing-authorized-cau-complete-batch-01-${reviewHash}`) throw new Error("CAU review hash/reference mismatch.");
const fresh = createCatalogMigrationValidationReport(candidate);
if (!fresh.ok || !validation.ok || sha(candidateText) !== review.candidateSha256 || fresh.bundleSha256 !== review.candidateBundleSha256 || fresh.bundleSha256 !== validation.bundleSha256 || fresh.operationPlanSha256 !== review.operationPlanSha256 || fresh.operationPlanSha256 !== validation.operationPlanSha256) throw new Error("CAU candidate changed after review.");
if (review.status !== "standing_user_approval" || review.standingAuthorization?.instruction !== "发布默认允许" || review.sensitiveDataCheck?.result !== "pass" || review.sourceReview?.result !== "pass") throw new Error("CAU review is not eligible for standing publication.");
if (review.scope?.newSchoolCount !== 1 || review.scope?.programRouteCount !== 127 || review.scope?.programIntakeCount !== 127 || review.scope?.newScholarshipCount !== 4 || review.scope?.cityOverwriteCount !== 0 || review.scope?.schoolOverwriteCount !== 0 || review.scope?.archiveProgramAliasCount !== 0 || review.scope?.archiveScholarshipAliasCount !== 0 || review.reconciliation?.destructiveDeletion !== false) throw new Error("Standing authorization cannot overwrite, archive or delete records.");
...
const publication: CatalogSeedBundle = {
  ...
  handoff: { sourceSystem: "CUAC reviewed official China Agricultural University 2026 sources", cleanedExportName: "catalog.cau-complete-batch-01.draft.json", cleanedExportSha256: fresh.bundleSha256, sourceSchemaSha256, reviewReference: publicationReference, prohibitedDataReviewReference: publicationReference, approvalRecordedAt: approvedAt, sourceReadOnly: true, prohibitedDataConfirmedExcluded: true },
...
if (!report.ok || report.summary.schools !== 1 || report.summary.programs !== 127 || report.summary.programIntakes !== 127 || report.summary.scholarships !== 4) throw new Error(`Invalid CAU publication: ${report.errors.join(" ")}`);
const approval = { version: 1, approvedAt, approvalMode: "standing-user-default-publication", reviewReference: publicationReference, approvedReviewSha256: reviewHash, approvedCandidateFileSha256: sha(candidateText), approvedCandidateBundleSha256: fresh.bundleSha256, publicationBundle: "seeds/catalog.cau-complete-batch-01.approved.local.json", publicationBundleSha256: report.bundleSha256, schoolSlug: "china-agricultural-university", programSlugs: publication.programs!.map((row) => row.slug), scholarshipSlugs: publication.scholarships!.map((row) => row.slug), archivedProgramAliases: [], archivedScholarshipAliases: [], prohibitedDataConfirmedExcluded: true };
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:cau-complete-publish" });
try {
  ...
  const result = await client.transaction(async (tx) => {
    ...
    const schools = await tx.query<any>("select id,status,verification_status,last_verified_at from schools where slug=$1 or lower(name_en)=lower($2) or name_zh=$3", ["china-agricultural-university", "China Agricultural University", "中国农业大学"]);
    ...
    const scholarshipRows = await tx.query<any>("select slug,status,verification_status,last_verified_at from scholarships where slug=any($1::text[])", [scholarshipSlugs]);
    const pre = schools.length === 0 && programRows.length === 0 && scholarshipRows.length === 0;
    const post = schools.length === 1 && schools[0].status === "active" && schools[0].verification_status === "verified" && schools[0].last_verified_at?.toISOString() === approvedAt && programRows.length === 127 && programRows.every((row) => row.status === "active" && row.verification_status === "verified" && row.last_verified_at?.toISOString() === approvedAt) && scholarshipRows.length === 4 && scholarshipRows.every((row) => row.status === "active" && row.verification_status === "verified" && row.last_verified_at?.toISOString() === approvedAt);
    if (!pre && !post) throw new Error(`CAU candidate scope partially conflicts: ${JSON.stringify({ schools: schools.length, programs: programRows.length, scholarships: scholarshipRows.length })}`);
    ...
    const afterSchool = await tx.query<any>("select id,status,verification_status,last_verified_at from schools where slug='china-agricultural-university'", []);
    const totals = await tx.query<any>("select (select count(*) from programs where school_id=$1 and status='active')::text programs,(select count(*) from programs where school_id=$1 and verification_status='verified')::text verified_programs,(select count(*) from program_intakes pi join programs p on p.id=pi.program_id where p.school_id=$1)::text intakes,(select count(*) from scholarships where school_id=$1 and status='active')::text scholarships,(select count(*) from scholarships where school_id=$1 and verification_status='verified')::text verified_scholarships", [afterSchool[0]?.id]);
    if (afterSchool.length !== 1 || afterSchool[0].verification_status !== "verified" || totals[0]?.programs !== "127" || totals[0]?.verified_programs !== "127" || totals[0]?.intakes !== "127" || totals[0]?.scholarships !== "4" || totals[0]?.verified_scholarships !== "4") throw new Error(`CAU post-publication totals invalid: ${JSON.stringify(totals[0])}`);
