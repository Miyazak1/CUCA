...
const paths = {
  candidate: resolve(root, "seeds/catalog.sysu-complete-batch-01.draft.json"),
  validation: resolve(root, "seeds/catalog.sysu-complete-batch-01.validation.json"),
  review: resolve(root, "seeds/catalog.sysu-complete-batch-01.review.json"),
  output: resolve(root, "seeds/catalog.sysu-complete-batch-01.approved.local.json"),
  approval: resolve(root, "seeds/catalog.sysu-complete-batch-01.approval.json"),
...
if (storedReviewHash !== approvedReviewHash || actualReviewHash !== approvedReviewHash) throw new Error("Standing-authorized review hash does not match the stored SYSU review.");
if (createHash("sha256").update(candidateText).digest("hex") !== review.candidateSha256) throw new Error("SYSU candidate changed after review.");
const freshValidation = createCatalogMigrationValidationReport(candidate);
if (!freshValidation.ok || !storedValidation.ok || freshValidation.bundleSha256 !== storedValidation.bundleSha256 || freshValidation.operationPlanSha256 !== storedValidation.operationPlanSha256) throw new Error("SYSU candidate validation no longer matches the reviewed bundle.");
if (review.status !== "standing_user_approval" || review.standingAuthorization?.instruction !== "发布默认允许" || review.sensitiveDataCheck?.result !== "pass") throw new Error("SYSU review is not eligible for standing-authorized publication.");
if (review.scope?.schoolSlug !== "sun-yat-sen-university" || review.scope?.schoolCount !== 1 || review.scope?.programRouteCount !== 729 || review.scope?.scholarshipCount !== 10) throw new Error("SYSU review scope mismatch.");
...
const publication: CatalogSeedBundle = {
  ...
  handoff: {
    sourceSystem: "CUAC reviewed official Sun Yat-sen University 2026 sources",
    cleanedExportName: "catalog.sysu-complete-batch-01.draft.json",
...
if (!publicationReport.ok || publicationReport.summary.schools !== 1 || publicationReport.summary.programs !== 729 || publicationReport.summary.programIntakes !== 729 || publicationReport.summary.scholarships !== 10) throw new Error(`SYSU publication bundle is invalid: ${publicationReport.errors.join(" ")}`);
const approvalRecord = {
  version: 1, approvedAt: approvalRecordedAt, reviewReference, approvedReviewSha256: approvedReviewHash,
  approvalMode: "standing-user-default-publication", approvedCandidate: "seeds/catalog.sysu-complete-batch-01.draft.json",
  approvedCandidateBundleSha256: freshValidation.bundleSha256, publicationBundle: "seeds/catalog.sysu-complete-batch-01.approved.local.json",
  publicationBundleSha256: publicationReport.bundleSha256, schoolSlug: "sun-yat-sen-university",
  programSlugs: publication.programs?.map(row => row.slug), scholarshipSlugs: publication.scholarships?.map(row => row.slug),
  archivedProgramAliases: [], archivedScholarshipAliases: [], prohibitedDataConfirmedExcluded: true,
};
async function writeOrVerify(path: string, value: unknown) {
  ...
  try { await writeFile(path, text, { encoding: "utf8", flag: "wx" }); }
    ...
    if (await readFile(path, "utf8") !== text) throw new Error(`Existing artifact does not match this publication: ${path}`);
  }
}
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:sysu-complete-publish" });
try {
  ...
  const result = await client.transaction(async tx => {
    ...
    const before = await tx.query<{ id: string }>("select id from schools where slug=$1 or lower(name_en)=lower($2) or name_zh=$3", ["sun-yat-sen-university", "Sun Yat-sen University", "中山大学"]);
    if (before.length !== 0) throw new Error("Sun Yat-sen University now exists; standing authorization requires a new conflict review before overwrite.");
    ...
    const counts = await tx.query<{ programs: string; intakes: string; scholarships: string }>("select (select count(*) from programs p where p.school_id=s.id and p.status='active')::text programs,(select count(*) from program_intakes pi join programs p on p.id=pi.program_id where p.school_id=s.id)::text intakes,(select count(*) from scholarships sc where sc.school_id=s.id and sc.status='active' and sc.verification_status='verified')::text scholarships from schools s where s.slug=$1", ["sun-yat-sen-university"]);
    if (counts[0]?.programs !== "729" || counts[0]?.intakes !== "729" || counts[0]?.scholarships !== "10") throw new Error(`SYSU publication count mismatch: ${JSON.stringify(counts[0])}`);
    return { written, activeProgramCount: 729, intakeCount: 729, activeVerifiedScholarshipCount: 10 };
  });
