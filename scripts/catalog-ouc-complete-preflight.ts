...
const pool = createPostgresPool({ databaseUrl: localDatabaseUrl(state), max: 1, applicationName: "cuac:ouc-complete-preflight" });
try {
  const schools = await pool.query(`select id,slug,name_en,name_zh,status,verification_status,source_url,city_id
    from schools where slug='ocean-university-of-china' or lower(name_en)=lower('Ocean University of China') or name_zh='中国海洋大学' order by slug`);
  const schoolIds = schools.rows.map((row) => row.id);
  const programs = schoolIds.length ? await pool.query(`select id,slug,name_en,degree_level,teaching_language,tuition_text,status,verification_status,source_url
  ...
  const city = await pool.query("select id,slug,name_en,name_zh,region,province,status,verification_status,source_url from cities where slug='qingdao'");
