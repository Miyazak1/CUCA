...
const candidatePath = resolve(root, "seeds/catalog.bit-scholarships-batch-01.draft.json");
...
const outputPath = resolve(root, "seeds/catalog.bit-scholarships-batch-01.approved.local.json");
const approvalPath = resolve(root, "seeds/catalog.bit-scholarships-batch-01.approval.json");
...
if (actualReviewHash !== approvedReviewHash) throw new Error("User-approved review hash does not match the stored BIT scholarship review.");
...
if (!freshValidation.ok || freshValidation.bundleSha256 !== approvedCandidateHash) throw new Error("Stored BIT scholarship review does not match a valid candidate bundle.");
if (!storedValidation.ok || storedValidation.bundleSha256 !== approvedCandidateHash || storedValidation.operationPlanSha256 !== freshValidation.operationPlanSha256) throw new Error("Stored validation does not match the approved BIT scholarship candidate bundle.");
if (review.status !== "awaiting_user_approval" || review.candidateBundleSha256 !== approvedCandidateHash) throw new Error("BIT scholarship review is not awaiting approval for this candidate.");
if (review.sensitiveDataCheck?.result !== "pass" || review.sensitiveDataCheck?.scope !== "candidate bundle only") throw new Error("BIT scholarship candidate has not passed prohibited-data review.");
if (review.scope?.schoolSlug !== "beijing-institute-of-technology" || review.scope?.schoolCount !== 1 || review.scope?.scholarshipCount !== 6 || review.scope?.existingVerifiedCount !== 1 || review.scope?.newScholarshipCount !== 5) throw new Error("BIT scholarship review scope must contain one school, one stable scholarship and five new scholarships.");

const scholarships = candidate.scholarships ?? [];
const scholarshipSlugs = scholarships.map(item => item.slug);
if (scholarships.length !== 6 || new Set(scholarshipSlugs).size !== 6 || scholarships.some(item => item.schoolSlug !== "beijing-institute-of-technology" || item.status !== "draft")) throw new Error("BIT scholarship candidate must contain six distinct draft scholarships for BIT.");
...
if (!Array.isArray(stableMatchingSlugs) || stableMatchingSlugs.length !== 1 || stableMatchingSlugs[0] !== "official-2026-bit-chinese-government-scholarship") throw new Error("BIT scholarship reconciliation must retain the reviewed CSC Type B record.");
if (!Array.isArray(legacyAliases) || legacyAliases.length !== 0 || review.reconciliation?.destructiveDeletion !== false) throw new Error("BIT scholarship review must not archive or delete records.");
...
const publication: CatalogSeedBundle = {
  ...
  handoff: {
    sourceSystem: "CUAC reviewed official Beijing Institute of Technology 2026 admission and scholarship book",
    cleanedExportName: "catalog.bit-scholarships-batch-01.draft.json",
...
if (!publicationReport.ok || publicationReport.summary.cities !== 2 || publicationReport.summary.schools !== 1 || publicationReport.summary.programs !== 0 || publicationReport.summary.programIntakes !== 0 || publicationReport.summary.scholarships !== 6) throw new Error(`BIT scholarship publication bundle is invalid: ${publicationReport.errors.join(" ")}`);
...
const approvalRecord = {
  ...
  approvedCandidate: "seeds/catalog.bit-scholarships-batch-01.draft.json",
  approvedCandidateBundleSha256: approvedCandidateHash,
  publicationBundle: "seeds/catalog.bit-scholarships-batch-01.approved.local.json",
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:bit-scholarships-publish" });
try {
  ...
  const result = await client.transaction(async tx => {
    ...
    if (schoolRows.length !== 1 || schoolRows[0]?.status !== "active" || schoolRows[0]?.verification_status !== "verified" || schoolRows[0]?.program_count !== "21") throw new Error("BIT no longer matches the reviewed verified school with 21 active programs.");
    ...
    const isReviewedPreState = beforeRows.length === 1 && beforeRows[0]?.slug === stableMatchingSlugs[0] && beforeRows[0]?.status === "active" && beforeRows[0]?.verification_status === "verified";
    const isApprovedPostState = beforeRows.length === 6 && beforeRows.every(row => row.status === "active" && row.verification_status === "verified" && row.last_verified_at?.toISOString() === approvalRecordedAt);
    if (!isReviewedPreState && !isApprovedPostState) throw new Error("BIT scholarships no longer match either the reviewed pre-publication state or this approval's published state.");
    ...
    if (finalRows.length !== 6 || finalRows.some(row => row.verification_status !== "verified" || row.last_verified_at?.toISOString() !== approvalRecordedAt || Number(row.body_count) < 1 || Number(row.action_count) < 1)) throw new Error("Post-publication BIT scholarship verification or rich-detail checks failed.");
    return { written, scholarshipSlugs: finalRows.map(row => row.slug), finalScholarshipCount: finalRows.length };
  });
