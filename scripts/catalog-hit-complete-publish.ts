...
const paths = {
  candidate: resolve(root, "seeds/catalog.hit-complete-batch-01.draft.json"),
  validation: resolve(root, "seeds/catalog.hit-complete-batch-01.validation.json"),
  review: resolve(root, "seeds/catalog.hit-complete-batch-01.review.json"),
  output: resolve(root, "seeds/catalog.hit-complete-batch-01.approved.local.json"),
  approval: resolve(root, "seeds/catalog.hit-complete-batch-01.approval.json"),
...
if (storedReviewHash !== approvedReviewHash || actualReviewHash !== approvedReviewHash) throw new Error("Standing-authorized review hash does not match the stored HIT review.");
if (createHash("sha256").update(candidateText).digest("hex") !== review.candidateSha256) throw new Error("HIT candidate changed after review.");
const freshValidation = createCatalogMigrationValidationReport(candidate);
if (!freshValidation.ok || !storedValidation.ok || freshValidation.bundleSha256 !== storedValidation.bundleSha256 || freshValidation.operationPlanSha256 !== storedValidation.operationPlanSha256) throw new Error("HIT candidate validation no longer matches the reviewed bundle.");
if (review.status !== "standing_user_approval" || review.standingAuthorization?.instruction !== "发布默认允许" || review.sensitiveDataCheck?.result !== "pass") throw new Error("HIT review is not eligible for standing-authorized publication.");
if (review.scope?.schoolSlug !== "harbin-institute-of-technology" || review.scope?.schoolCount !== 1 || review.scope?.programRouteCount !== 182 || review.scope?.scholarshipCount !== 4) throw new Error("HIT review scope mismatch.");
...
const publication: CatalogSeedBundle = {
  ...
  handoff: {
    sourceSystem: "CUAC reviewed official Harbin Institute of Technology 2026 sources",
    cleanedExportName: "catalog.hit-complete-batch-01.draft.json",
...
if (!publicationReport.ok || publicationReport.summary.schools !== 1 || publicationReport.summary.programs !== 182 || publicationReport.summary.programIntakes !== 182 || publicationReport.summary.scholarships !== 4) throw new Error(`HIT publication bundle is invalid: ${publicationReport.errors.join(" ")}`);
const approvalRecord = {
  version: 1, approvedAt: approvalRecordedAt, reviewReference, approvedReviewSha256: approvedReviewHash,
  approvalMode: "standing-user-default-publication", approvedCandidate: "seeds/catalog.hit-complete-batch-01.draft.json",
  approvedCandidateBundleSha256: freshValidation.bundleSha256, publicationBundle: "seeds/catalog.hit-complete-batch-01.approved.local.json",
  publicationBundleSha256: publicationReport.bundleSha256, schoolSlug: "harbin-institute-of-technology",
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:hit-complete-publish" });
try {
  ...
  const result = await client.transaction(async tx => {
    ...
    const before = await tx.query<{ id: string }>("select id from schools where slug=$1 or lower(name_en)=lower($2) or name_zh=$3", ["harbin-institute-of-technology", "Harbin Institute of Technology", "哈尔滨工业大学"]);
    if (before.length !== 0) throw new Error("HIT main-campus record now exists; standing authorization requires a new conflict review before overwrite.");
