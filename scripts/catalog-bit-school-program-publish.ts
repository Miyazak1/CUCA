...
const outputPath = resolve(root, "seeds/catalog.bit-school-program-batch-01.approved.local.json");
const approvalPath = resolve(root, "seeds/catalog.bit-school-program-batch-01.approval.json");
...
if (actualReviewHash !== approvedReviewHash) throw new Error("User-approved review hash does not match the stored Beijing Institute of Technology review.");
...
if (!freshValidation.ok || freshValidation.bundleSha256 !== approvedCandidateHash) throw new Error("Stored BIT review does not match a valid candidate bundle.");
if (!storedValidation.ok || storedValidation.bundleSha256 !== approvedCandidateHash || storedValidation.operationPlanSha256 !== freshValidation.operationPlanSha256) throw new Error("Stored validation does not match the approved BIT candidate bundle.");
if (review.status !== "awaiting_user_approval" || review.candidateBundleSha256 !== approvedCandidateHash) throw new Error("BIT review is not awaiting approval for this candidate.");
if (review.sensitiveDataCheck?.result !== "pass" || review.sensitiveDataCheck?.scope !== "candidate bundle only") throw new Error("BIT candidate has not passed prohibited-data review.");
if (
  review.scope?.schoolSlug !== "beijing-institute-of-technology" || review.scope?.schoolCount !== 1 ||
  review.scope?.applicationRouteCount !== 21 || review.scope?.programCount !== 21 || review.scope?.intakeCount !== 21 ||
  review.scope?.chineseCategoryCount !== 8 || review.scope?.chineseRussianZhuhaiCount !== 5 || review.scope?.englishProgramCount !== 8 ||
  review.scope?.beijingProgramCount !== 15 || review.scope?.zhuhaiProgramCount !== 6
) throw new Error("BIT review scope must contain exactly one school, 21 language/campus-qualified routes, and 21 intakes.");
...
if (!Array.isArray(legacyAliases) || legacyAliases.length !== 10 || new Set(legacyAliases).size !== 10) throw new Error("BIT reconciliation must contain ten distinct legacy aliases.");
if (!Array.isArray(stableMatchingSlugs) || stableMatchingSlugs.length !== 7 || new Set(stableMatchingSlugs).size !== 7) throw new Error("BIT reconciliation must contain seven distinct stable matches.");
if ((candidate.programs ?? []).some(program => legacyAliases.includes(program.slug))) throw new Error("A BIT archive record overlaps the approved current catalog.");
if (stableMatchingSlugs.some(slug => !(candidate.programs ?? []).some(program => program.slug === slug))) throw new Error("A reviewed BIT stable match is absent from the approved candidate.");

const publication: CatalogSeedBundle = {
  version: 2,
  generatedAt: approvalRecordedAt,
  handoff: {
    sourceSystem: "CUAC reviewed official Beijing Institute of Technology 2026 admissions and program catalog sources",
    cleanedExportName: "catalog.bit-school-program-batch-01.draft.json",
...
if (!publicationReport.ok || publicationReport.summary.cities !== 2 || publicationReport.summary.schools !== 1 || publicationReport.summary.programs !== 21 || publicationReport.summary.programIntakes !== 21 || publicationReport.summary.scholarships !== 0) throw new Error(`BIT publication bundle is invalid: ${publicationReport.errors.join(" ")}`);
const approvalRecord = {
  version: 1,
  approvedAt: approvalRecordedAt,
  reviewReference,
  approvedReviewSha256: approvedReviewHash,
  approvedCandidate: "seeds/catalog.bit-school-program-batch-01.draft.json",
  approvedCandidateBundleSha256: approvedCandidateHash,
  publicationBundle: "seeds/catalog.bit-school-program-batch-01.approved.local.json",
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:bit-school-program-publish" });
try {
  ...
  const result = await client.transaction(async tx => {
    ...
    if (schoolRows.length !== 1 || schoolRows[0]?.status !== "active" || schoolRows[0]?.verification_status !== "unverified" || schoolRows[0]?.program_count !== "17") throw new Error("BIT no longer matches the reviewed active/unverified school with seventeen active programs.");
    const reviewedRows = await tx.query<{ slug: string; status: string; verification_status: string }>(
      ...
      ["beijing-institute-of-technology", [...stableMatchingSlugs, ...legacyAliases]],
    );
    if (reviewedRows.length !== 17 || reviewedRows.some(row => row.status !== "active" || row.verification_status !== "unverified")) throw new Error("The seven stable matches and ten approved archive records no longer match the reviewed BIT state.");
    ...
    const archived = await tx.query<{ slug: string }>(
      ...
      ["beijing-institute-of-technology", legacyAliases],
    );
    if (archived.length !== 10 || archived.some(row => !legacyAliases.includes(row.slug))) throw new Error("Approved BIT archive count changed during publication.");
    ...
    if (finalRows[0]?.program_count !== "21" || finalRows[0]?.verified_count !== "21") throw new Error("Post-publication BIT program counts are not exactly 21 verified active records.");
    return { written, stableMatches: stableMatchingSlugs.slice().sort(), archived: archived.map(row => row.slug).sort(), finalProgramCount: 21 };
  });
