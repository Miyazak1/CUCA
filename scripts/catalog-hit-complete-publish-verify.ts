...
const bundle = JSON.parse(await readFile(resolve(root, "seeds/catalog.hit-complete-batch-01.approved.local.json"), "utf8"));
const approval = JSON.parse(await readFile(resolve(root, "seeds/catalog.hit-complete-batch-01.approval.json"), "utf8"));
const reportPath = resolve(root, "seeds/catalog.hit-complete-batch-01.publication-verification.json");
...
const schoolResponse = await fetch(`${baseUrl}/api/v1/catalog/schools?limit=100&query=Harbin%20Institute%20of%20Technology`);
assert.equal(schoolResponse.status, 200);
const schools = (await schoolResponse.json() as { data: any[] }).data.filter(row => row.slug === "harbin-institute-of-technology");
assert.equal(schools.length, 1);
const school = schools[0];
assert.equal(school.programCount, 182);
...
assert.equal(programs.length, 182);
assert.deepEqual(new Set(programs.map(row => row.slug)), new Set(bundle.programs.map((row: any) => row.slug)));
const samples = [programs.find(row => row.degreeLevel === "Undergraduate"), programs.find(row => row.degreeLevel === "Master" && row.teachingLanguage === "Chinese"), programs.find(row => row.degreeLevel === "Master" && row.teachingLanguage === "English"), programs.find(row => row.degreeLevel === "Doctoral")].filter(Boolean);
