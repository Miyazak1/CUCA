...
const paths = {
  candidate: resolve(root, "seeds/catalog.jilin-safe-new-batch-01.draft.json"),
  ...
  output: resolve(root, "seeds/catalog.jilin-safe-new-batch-01.approved.local.json"),
  approval: resolve(root, "seeds/catalog.jilin-safe-new-batch-01.approval.json"),
...
if (storedHash !== approvedReviewHash || createHash("sha256").update(JSON.stringify(reviewBase)).digest("hex") !== approvedReviewHash || storedReference !== reviewReference) throw new Error("Standing-authorized Jilin review hash/reference mismatch.");
...
if (candidateSha256 !== review.candidateSha256 || !fresh.ok || !validation.ok || fresh.bundleSha256 !== validation.bundleSha256 || fresh.operationPlanSha256 !== validation.operationPlanSha256) throw new Error("Jilin candidate or validation changed after review.");
if (review.status !== "standing_user_approval" || review.standingAuthorization?.instruction !== "发布默认允许" || review.sensitiveDataCheck?.result !== "pass" || review.visualReview?.result !== "pass") throw new Error("Jilin review is not eligible for standing-authorized publication.");
if (review.scope?.programRouteCount !== 244 || review.scope?.programIntakeCount !== 244 || review.scope?.newScholarshipCount !== 5 || review.scope?.archiveProgramAliasCount !== 0 || review.scope?.archiveScholarshipAliasCount !== 0 || review.scope?.cityOverwriteCount !== 0 || review.scope?.schoolOverwriteCount !== 0 || review.reconciliation?.destructiveDeletion !== false) throw new Error("Standing authorization cannot archive, delete or substantively overwrite records.");

const programs = candidate.programs ?? [];
const intakes = candidate.programIntakes ?? [];
const scholarships = candidate.scholarships ?? [];
if (programs.length !== 244 || intakes.length !== 244 || scholarships.length !== 5 || programs.some(row => row.schoolSlug !== "jilin-university" || row.status !== "draft") || scholarships.some(row => row.schoolSlug !== "jilin-university" || row.status !== "draft")) throw new Error("Jilin candidate scope mismatch.");
...
const publication: CatalogSeedBundle = {
  ...
  handoff: { sourceSystem: "CUAC reviewed official Jilin University 2026/2027 sources", cleanedExportName: "catalog.jilin-safe-new-batch-01.draft.json", cleanedExportSha256: fresh.bundleSha256, sourceSchemaSha256, reviewReference, prohibitedDataReviewReference: reviewReference, approvalRecordedAt: approvedAt, sourceReadOnly: true, prohibitedDataConfirmedExcluded: true },
...
if (!report.ok || report.summary.programs !== 244 || report.summary.programIntakes !== 244 || report.summary.scholarships !== 5) throw new Error(`Invalid Jilin publication: ${report.errors.join(" ")}`);
const approval = { version: 1, approvedAt, approvalMode: "standing-user-default-publication", reviewReference, approvedReviewSha256: approvedReviewHash, approvedCandidateFileSha256: candidateSha256, approvedCandidateBundleSha256: fresh.bundleSha256, publicationBundle: "seeds/catalog.jilin-safe-new-batch-01.approved.local.json", publicationBundleSha256: report.bundleSha256, schoolSlug: "jilin-university", programSlugs: publication.programs!.map(row => row.slug), scholarshipSlugs: publication.scholarships!.map(row => row.slug), archivedProgramAliases: [], archivedScholarshipAliases: [], prohibitedDataConfirmedExcluded: true };
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:jilin-safe-new-batch-01" });
try {
  ...
  const result = await client.transaction(async tx => {
    ...
    const school = await tx.query<{ id: string }>("select id from schools where slug=$1 and status='active'", ["jilin-university"]);
    if (school.length !== 1 || school[0].id !== "521a64e8-34f1-478a-98ab-0a5a1e2e1f27") throw new Error("Jilin dependency school mismatch.");
    ...
    const post = existingPrograms.length === 244 && existingPrograms.every(row => row.status === "active" && row.verification_status === "verified") && existingScholarships.length === 5 && existingScholarships.every(row => row.status === "active" && row.verification_status === "verified");
    if (!pre && !post) throw new Error(`Jilin candidate slugs partially conflict: ${JSON.stringify({ programs: existingPrograms.length, scholarships: existingScholarships.length })}`);
    ...
    if (finalPrograms[0]?.count !== "244" || finalIntakes[0]?.count !== "244" || finalScholarships[0]?.count !== "5") throw new Error(`Jilin post-check failed: ${JSON.stringify({ finalPrograms, finalIntakes, finalScholarships })}`);
    ...
    if (totals[0]?.programs !== "247" || totals[0]?.intakes !== "247" || totals[0]?.scholarships !== "7" || totals[0]?.verified_scholarships !== "6") throw new Error(`Jilin total-count mismatch: ${JSON.stringify(totals[0])}`);
