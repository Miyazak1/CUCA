...
const outputPath = resolve(root, "seeds/catalog.zju-school-program-batch-01.approved.local.json");
const approvalPath = resolve(root, "seeds/catalog.zju-school-program-batch-01.approval.json");
...
if (!freshValidation.ok || freshValidation.bundleSha256 !== approvedHash) throw new Error("Approved hash does not match a valid ZJU candidate bundle.");
...
if (review.status !== "awaiting_user_approval" || review.candidateBundleSha256 !== approvedHash) throw new Error("ZJU review is not awaiting approval for this candidate hash.");
if (review.sensitiveDataCheck?.result !== "pass" || review.sensitiveDataCheck?.scope !== "candidate bundle only") throw new Error("ZJU candidate has not passed prohibited-data review.");
if (review.scope?.schoolCount !== 1 || review.scope?.programCount !== 92 || review.scope?.intakeCount !== 92 || review.scope?.chineseProgramCount !== 87 || review.scope?.englishProgramCount !== 5) {
  throw new Error("ZJU review scope must contain exactly one school, 92 programs and 92 intakes.");
}
const legacyAliases = review.reconciliation?.legacyAliasesToArchive;
if (!Array.isArray(legacyAliases) || legacyAliases.length !== 8 || new Set(legacyAliases).size !== 8) throw new Error("ZJU reconciliation must contain eight distinct legacy aliases.");
...
const publication: CatalogSeedBundle = {
  ...
  handoff: {
    sourceSystem: "CUAC reviewed official Zhejiang University 2026 web and PDF sources",
    cleanedExportName: "catalog.zju-school-program-batch-01.draft.json",
...
if (!publicationReport.ok || publicationReport.summary.schools !== 1 || publicationReport.summary.programs !== 92 || publicationReport.summary.programIntakes !== 92) {
...
const approvalRecord = {
  ...
  reviewReference,
  approvedCandidate: "seeds/catalog.zju-school-program-batch-01.draft.json",
  approvedCandidateBundleSha256: approvedHash,
  publicationBundle: "seeds/catalog.zju-school-program-batch-01.approved.local.json",
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:zju-school-program-publish" });
try {
  ...
  const result = await client.transaction(async tx => {
    ...
    const aliasRows = await tx.query<{ slug: string; status: string; verification_status: string }>(
      ...
      ["zhejiang-university", legacyAliases],
    );
    if (aliasRows.length !== legacyAliases.length || aliasRows.some(row => row.status !== "active" || row.verification_status !== "unverified")) {
      throw new Error("The eight approved aliases no longer exactly match the reviewed active/unverified state.");
    }
