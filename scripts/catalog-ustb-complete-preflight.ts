...
const pool = createPostgresPool({ databaseUrl: localDatabaseUrl(state), max: 1, applicationName: "cuac:ustb-complete-preflight" });

try {
  const schools = await pool.query(`select id,slug,name_en,name_zh,status,verification_status,source_url,city_id
    from schools where slug='university-of-science-and-technology-beijing'
      or lower(name_en)=lower('University of Science and Technology Beijing')
      or name_zh='北京科技大学' order by slug`);
  ...
  const city = await pool.query("select id,slug,name_en,name_zh,region,province,status,verification_status,source_url from cities where slug='beijing'");
