...
const paths = {
  candidate: resolve(root, "seeds/catalog.dut-safe-new-batch-01.draft.json"),
  ...
  output: resolve(root, "seeds/catalog.dut-safe-new-batch-01.approved.local.json"),
  approval: resolve(root, "seeds/catalog.dut-safe-new-batch-01.approval.json"),
...
if (storedHash !== approvedReviewHash || createHash("sha256").update(JSON.stringify(reviewBase)).digest("hex") !== approvedReviewHash || storedReference !== reviewReference) throw new Error("Standing-authorized DUT review hash/reference mismatch.");
...
if (candidateSha256 !== review.candidateSha256 || !fresh.ok || !validation.ok || fresh.bundleSha256 !== validation.bundleSha256 || fresh.operationPlanSha256 !== validation.operationPlanSha256) throw new Error("DUT candidate or validation changed after review.");
if (review.status !== "standing_user_approval" || review.standingAuthorization?.instruction !== "发布默认允许" || review.sensitiveDataCheck?.result !== "pass" || review.visualReview?.result !== "pass") throw new Error("DUT review is not eligible for standing-authorized publication.");
if (review.scope?.programRouteCount !== 3 || review.scope?.programIntakeCount !== 3 || review.scope?.newScholarshipCount !== 4 || review.scope?.archiveProgramAliasCount !== 0 || review.scope?.archiveScholarshipAliasCount !== 0 || review.scope?.cityOverwriteCount !== 0 || review.scope?.schoolOverwriteCount !== 0 || review.reconciliation?.destructiveDeletion !== false) throw new Error("Standing authorization cannot archive, delete or substantively overwrite records.");

const programs = candidate.programs ?? [];
const intakes = candidate.programIntakes ?? [];
const scholarships = candidate.scholarships ?? [];
if (programs.length !== 3 || intakes.length !== 3 || scholarships.length !== 4 || programs.some(row => row.schoolSlug !== "dalian-university-of-technology" || row.status !== "draft") || scholarships.some(row => row.schoolSlug !== "dalian-university-of-technology" || row.status !== "draft")) throw new Error("DUT candidate scope mismatch.");
const sourceSchemaSha256 = createHash("sha256").update(JSON.stringify({ schoolFields: [...new Set((candidate.schools ?? []).flatMap(row => Object.keys(row)))].sort(), programFields: [...new Set(programs.flatMap(row => Object.keys(row)))].sort(), intakeFields: [...new Set(intakes.flatMap(row => Object.keys(row)))].sort(), scholarshipFields: [...new Set(scholarships.flatMap(row => Object.keys(row)))].sort() })).digest("hex");
const publication: CatalogSeedBundle = {
  version: 2,
  generatedAt: approvedAt,
  handoff: { sourceSystem: "CUAC reviewed official DUT 2026 sources", cleanedExportName: "catalog.dut-safe-new-batch-01.draft.json", cleanedExportSha256: fresh.bundleSha256, sourceSchemaSha256, reviewReference, prohibitedDataReviewReference: reviewReference, approvalRecordedAt: approvedAt, sourceReadOnly: true, prohibitedDataConfirmedExcluded: true },
...
if (!report.ok || report.summary.programs !== 3 || report.summary.programIntakes !== 3 || report.summary.scholarships !== 4) throw new Error(`Invalid DUT publication: ${report.errors.join(" ")}`);
const approval = { version: 1, approvedAt, approvalMode: "standing-user-default-publication", reviewReference, approvedReviewSha256: approvedReviewHash, approvedCandidateFileSha256: candidateSha256, approvedCandidateBundleSha256: fresh.bundleSha256, publicationBundle: "seeds/catalog.dut-safe-new-batch-01.approved.local.json", publicationBundleSha256: report.bundleSha256, schoolSlug: "dalian-university-of-technology", programSlugs: publication.programs!.map(row => row.slug), scholarshipSlugs: publication.scholarships!.map(row => row.slug), archivedProgramAliases: [], archivedScholarshipAliases: [], prohibitedDataConfirmedExcluded: true };
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:dut-safe-new-batch-01" });
try {
  ...
  const result = await client.transaction(async tx => {
    ...
    const school = await tx.query<{ id: string; source_url: string; verification_status: string }>("select id,source_url,verification_status from schools where slug=$1 and status='active'", ["dalian-university-of-technology"]);
    if (school.length !== 1 || school[0].id !== "513f6ff8-1e21-4066-9760-b1eba665363d" || school[0].source_url !== "https://csca.app/zh/universities/dalian-university-of-technology" || school[0].verification_status !== "unverified") throw new Error("DUT dependency school baseline mismatch.");
    ...
    const post = existingPrograms.length === 3 && existingPrograms.every(row => row.status === "active" && row.verification_status === "verified") && existingScholarships.length === 4 && existingScholarships.every(row => row.status === "active" && row.verification_status === "verified");
    if (!pre && !post) throw new Error(`DUT candidate slugs partially conflict: ${JSON.stringify({ programs: existingPrograms.length, scholarships: existingScholarships.length })}`);
    ...
    if (finalPrograms[0]?.count !== "3" || finalIntakes[0]?.count !== "3" || finalScholarships[0]?.count !== "4") throw new Error(`DUT post-check failed: ${JSON.stringify({ finalPrograms, finalIntakes, finalScholarships })}`);
    const schoolAfter = await tx.query<{ source_url: string; verification_status: string }>("select source_url,verification_status from schools where id=$1", [school[0].id]);
    if (schoolAfter[0]?.source_url !== school[0].source_url || schoolAfter[0]?.verification_status !== school[0].verification_status) throw new Error("DUT dependency school was substantively changed.");
    ...
    if (totals[0]?.programs !== "15" || totals[0]?.intakes !== "8" || totals[0]?.scholarships !== "7" || totals[0]?.verified_scholarships !== "5") throw new Error(`DUT total-count mismatch: ${JSON.stringify(totals[0])}`);
