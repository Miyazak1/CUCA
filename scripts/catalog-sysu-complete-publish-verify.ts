...
const bundle = JSON.parse(await readFile(resolve(root, "seeds/catalog.sysu-complete-batch-01.approved.local.json"), "utf8"));
const approval = JSON.parse(await readFile(resolve(root, "seeds/catalog.sysu-complete-batch-01.approval.json"), "utf8"));
const reportPath = resolve(root, "seeds/catalog.sysu-complete-batch-01.publication-verification.json");
...
const schoolResponse = await fetch(`${baseUrl}/api/v1/catalog/schools?limit=100&query=Sun%20Yat-sen%20University`);
assert.equal(schoolResponse.status, 200);
const schools = (await schoolResponse.json() as { data: any[] }).data.filter(row => row.slug === "sun-yat-sen-university");
assert.equal(schools.length, 1);
const school = schools[0];
assert.equal(school.programCount, 729);
assert.equal(school.scholarshipCount, 10);
...
const publicProgramSlugs = new Set(programs.map(row => row.slug));
const missingProgramSlugs = bundle.programs.map((row: any) => row.slug).filter((slug: string) => !publicProgramSlugs.has(slug));
if (missingProgramSlugs.length) console.error(JSON.stringify({ missingProgramSlugs }, null, 2));
assert.equal(programs.length, 729);
assert.deepEqual(publicProgramSlugs, new Set(bundle.programs.map((row: any) => row.slug)));
const samples = [programs[0], programs.find(row => row.degreeLevel === "Master"), programs.find(row => row.degreeLevel === "Doctoral"), programs.find(row => row.slug.includes("cambodia-aerospace"))].filter(Boolean);
...
assert.equal(scholarships.length, 10);
