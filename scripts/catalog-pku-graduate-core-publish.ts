...
if (!/^[a-f0-9]{64}$/.test(reviewHash) || publicationReference !== "standing-user-default-publication" || confirmation !== LOCAL_CATALOG_PUBLISH_CONFIRMATION) throw new Error("Invalid PKU graduate publication parameters.");
...
const paths = {
  ...
  output: resolve(root, "seeds/catalog.pku-graduate-core-batch-01.approved.local.json"),
  approval: resolve(root, "seeds/catalog.pku-graduate-core-batch-01.approval.json"),
...
if (storedReviewHash !== reviewHash || sha(JSON.stringify(reviewBase)) !== reviewHash || review.status !== "standing_user_approval" || review.scope?.newProgramRouteCount !== 42 || review.scope?.newProgramIntakeCount !== 42 || review.scope?.overwriteCount !== 0 || review.scope?.archiveCount !== 0 || review.reconciliation?.destructiveDeletion !== false || review.sensitiveDataCheck?.result !== "pass") throw new Error("PKU graduate standing review is invalid.");
const fresh = createCatalogMigrationValidationReport(candidate);
if (!fresh.ok || !validation.ok || sha(candidateText) !== review.candidateSha256 || fresh.bundleSha256 !== review.candidateBundleSha256 || fresh.bundleSha256 !== validation.bundleSha256 || fresh.operationPlanSha256 !== review.operationPlanSha256 || fresh.operationPlanSha256 !== validation.operationPlanSha256) throw new Error("PKU graduate candidate changed after review.");
const approvedAt = new Date().toISOString();
const reviewReference = `standing-user-default-publication-pku-graduate-core-${reviewHash}`;
...
const publication: CatalogSeedBundle = {
  ...
  handoff: { sourceSystem: "CUAC reviewed official Peking University 2026 graduate admissions and tuition sources", cleanedExportName: "catalog.pku-graduate-core-batch-01.draft.json", cleanedExportSha256: fresh.bundleSha256, sourceSchemaSha256, reviewReference, prohibitedDataReviewReference: reviewReference, approvalRecordedAt: approvedAt, sourceReadOnly: true, prohibitedDataConfirmedExcluded: true },
...
if (!report.ok || report.summary.programs !== 42 || report.summary.programIntakes !== 42 || report.summary.scholarships !== 0) throw new Error(`Invalid PKU graduate publication: ${report.errors.join(" ")}`);
const approval = { version: 1, approvedAt, approvalMode: "standing-product-owner-authorization", reviewReference, approvedReviewSha256: reviewHash, approvedCandidateBundleSha256: fresh.bundleSha256, publicationBundle: "seeds/catalog.pku-graduate-core-batch-01.approved.local.json", publicationBundleSha256: report.bundleSha256, schoolSlug: "peking-university", programSlugs, archivedAliases: [], prohibitedDataConfirmedExcluded: true };
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:pku-graduate-core-publish" });
try {
  ...
  const result = await client.transaction(async (tx) => {
    ...
    if (!written.ok) throw new Error(`PKU graduate catalog write failed: ${written.errors.join(" ")}`);
    ...
    if (final[0]?.active_count !== 65 || final[0]?.verified_count !== 65 || final[0]?.published_batch !== 42 || intakeFinal[0]?.count !== 42 || JSON.stringify(scholarshipsAfter[0]) !== JSON.stringify(scholarshipsBefore[0])) throw new Error(`PKU graduate post-check failed: ${JSON.stringify({ final: final[0], intakes: intakeFinal[0], scholarshipsBefore: scholarshipsBefore[0], scholarshipsAfter: scholarshipsAfter[0] })}`);
