...
const root = process.cwd(), batch = "heu-safe-new-batch-01", prefix = `catalog.${batch}`;
...
if (sha(JSON.stringify(reviewBase)) !== reviewHash || publicationReference !== `standing-authorized-${batch}-${reviewHash}`) throw new Error("HEU review hash/reference mismatch.");
const fresh = createCatalogMigrationValidationReport(candidate);
if (!fresh.ok || !validation.ok || sha(candidateText) !== review.candidateSha256 || fresh.bundleSha256 !== review.candidateBundleSha256 || fresh.bundleSha256 !== validation.bundleSha256 || fresh.operationPlanSha256 !== review.operationPlanSha256 || fresh.operationPlanSha256 !== validation.operationPlanSha256) throw new Error("HEU candidate changed after review.");
if (review.status !== "standing_user_approval" || review.standingAuthorization?.instruction !== "发布默认允许" || review.sourceReview?.result !== "pass" || review.sensitiveDataCheck?.result !== "pass" || review.reconciliation?.destructiveDeletion !== false) throw new Error("HEU review is not standing-authorized.");
if (review.scope?.newProgramRouteCount !== 87 || review.scope?.newProgramIntakeCount !== 87 || review.scope?.newScholarshipCount !== 10 || review.scope?.overwriteCount !== 0 || review.scope?.archiveCount !== 0) throw new Error("HEU standing scope cannot overwrite or archive records.");
const programs = candidate.programs ?? [], intakes = candidate.programIntakes ?? [], scholarships = candidate.scholarships ?? [];
if (candidate.cities?.length !== 1 || candidate.schools?.length !== 1 || programs.length !== 87 || intakes.length !== 87 || scholarships.length !== 10) throw new Error("HEU safe-new candidate scope mismatch.");
...
if (!report.ok || report.summary.programs !== 87 || report.summary.programIntakes !== 87 || report.summary.scholarships !== 10) throw new Error(`Invalid HEU publication: ${report.errors.join(" ")}`);
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:heu-safe-new-publish" });
try {
  ...
  const result = await client.transaction(async (tx) => {
    ...
    const schoolBefore = await tx.query<any>("select to_jsonb(s)-'created_at'-'updated_at' snapshot from schools s where slug='harbin-engineering-university'", []);
    if (cityBefore.length !== 1 || schoolBefore.length !== 1 || cityBefore[0].snapshot.id !== review.reconciliation.databaseBaseline.cityId || schoolBefore[0].snapshot.id !== review.reconciliation.databaseBaseline.schoolId || schoolBefore[0].snapshot.status !== "active" || schoolBefore[0].snapshot.verification_status !== "unverified") throw new Error("HEU dependency baseline changed.");
    ...
    if (deferredPrograms.length !== 9 || deferredPrograms.some((row) => row.status !== "active" || row.verification_status !== "unverified") || deferredScholarships.length !== 1 || deferredScholarships[0].status !== "active" || deferredScholarships[0].verification_status !== "unverified") throw new Error("HEU deferred legacy rows changed.");
    ...
    const post = beforePrograms.length === 87 && beforeScholarships.length === 10 && [...beforePrograms, ...beforeScholarships].every((row) => row.status === "active" && row.verification_status === "verified" && row.last_verified_at?.toISOString() === approvedAt);
    if (!pre && !post) throw new Error("HEU safe-new slugs partially conflict.");
    const written = await new CatalogSeedWriter(tx).writeBundle(publication, { preserveExistingCitySlugs: ["harbin"], preserveExistingSchoolSlugs: ["harbin-engineering-university"] });
    if (!written.ok) throw new Error(`HEU catalog write failed: ${written.errors.join(" ")}`);
    const cityAfter = await tx.query<any>("select to_jsonb(c)-'created_at'-'updated_at' snapshot from cities c where slug='harbin'", []);
    const schoolAfter = await tx.query<any>("select to_jsonb(s)-'created_at'-'updated_at' snapshot from schools s where slug='harbin-engineering-university'", []);
    if (JSON.stringify(cityAfter[0]?.snapshot) !== JSON.stringify(cityBefore[0].snapshot) || JSON.stringify(schoolAfter[0]?.snapshot) !== JSON.stringify(schoolBefore[0].snapshot)) throw new Error(`HEU dependency rows were substantively changed: ${JSON.stringify({ cityBefore: cityBefore[0]?.snapshot, cityAfter: cityAfter[0]?.snapshot, schoolBefore: schoolBefore[0]?.snapshot, schoolAfter: schoolAfter[0]?.snapshot })}`);
    ...
    const expected = { programs: review.reconciliation.databaseBaseline.activeProgramCount + 87, verified_programs: 87, intakes: review.reconciliation.databaseBaseline.intakeCount + 87, scholarships: review.reconciliation.databaseBaseline.activeScholarshipCount + 10, verified_rich_scholarships: 10 };
    if (Object.entries(expected).some(([key, value]) => totals[0]?.[key] !== value)) throw new Error(`HEU post-publication totals invalid: ${JSON.stringify({ expected, actual: totals[0] })}`);
