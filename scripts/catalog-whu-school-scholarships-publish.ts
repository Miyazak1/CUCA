...
const paths = {
  candidate: resolve(root, "seeds/catalog.whu-school-scholarships-batch-01.draft.json"),
  validation: resolve(root, "seeds/catalog.whu-school-scholarships-batch-01.validation.json"),
  review: resolve(root, "seeds/catalog.whu-school-scholarships-batch-01.review.json"),
  output: resolve(root, "seeds/catalog.whu-school-scholarships-batch-01.approved.local.json"),
  approval: resolve(root, "seeds/catalog.whu-school-scholarships-batch-01.approval.json"),
...
if (storedReviewHash !== approvedReviewHash || actualReviewHash !== approvedReviewHash) throw new Error("Standing-authorized review hash does not match the stored WHU review.");
if (createHash("sha256").update(candidateText).digest("hex") !== review.candidateSha256) throw new Error("WHU candidate changed after review.");
const freshValidation = createCatalogMigrationValidationReport(candidate);
if (!freshValidation.ok || !storedValidation.ok || freshValidation.bundleSha256 !== storedValidation.bundleSha256 || freshValidation.operationPlanSha256 !== storedValidation.operationPlanSha256) throw new Error("WHU candidate validation no longer matches the reviewed bundle.");
if (review.status !== "standing_user_approval" || review.standingAuthorization?.instruction !== "发布默认允许" || review.sensitiveDataCheck?.result !== "pass") throw new Error("WHU review is not eligible for standing-authorized publication.");
if (review.scope?.schoolSlug !== "wuhan-university" || review.scope?.schoolCount !== 1 || review.scope?.programRouteCount !== 0 || review.scope?.scholarshipCount !== 6) throw new Error("WHU review scope mismatch.");
...
const publication: CatalogSeedBundle = {
  ...
  handoff: {
    sourceSystem: "CUAC reviewed official Wuhan University 2026 sources",
    cleanedExportName: "catalog.whu-school-scholarships-batch-01.draft.json",
    cleanedExportSha256: freshValidation.bundleSha256, sourceSchemaSha256, reviewReference,
    prohibitedDataReviewReference: reviewReference, approvalRecordedAt, sourceReadOnly: true, prohibitedDataConfirmedExcluded: true,
  },
...
if (!publicationReport.ok || publicationReport.summary.schools !== 1 || publicationReport.summary.programs !== 0 || publicationReport.summary.scholarships !== 6) throw new Error(`WHU publication bundle is invalid: ${publicationReport.errors.join(" ")}`);
const approvalRecord = { version: 1, approvedAt: approvalRecordedAt, reviewReference, approvedReviewSha256: approvedReviewHash, approvalMode: "standing-user-default-publication", approvedCandidate: "seeds/catalog.whu-school-scholarships-batch-01.draft.json", approvedCandidateBundleSha256: freshValidation.bundleSha256, publicationBundle: "seeds/catalog.whu-school-scholarships-batch-01.approved.local.json", publicationBundleSha256: publicationReport.bundleSha256, schoolSlug: "wuhan-university", scholarshipSlugs: publication.scholarships?.map(row => row.slug), archivedProgramAliases: [], archivedScholarshipAliases: [], prohibitedDataConfirmedExcluded: true };
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:whu-school-scholarships-publish" });
try {
  ...
  const result = await client.transaction(async tx => {
    ...
    const before = await tx.query<{ id: string }>("select id from schools where slug=$1 or lower(name_en)=lower($2) or name_zh=$3", ["wuhan-university", "Wuhan University", "武汉大学"]);
    if (before.length !== 0) throw new Error("Wuhan University now exists; standing authorization requires a new conflict review before overwrite.");
    ...
    const counts = await tx.query<{ programs: string; scholarships: string }>("select (select count(*) from programs p where p.school_id=s.id and p.status='active')::text programs,(select count(*) from scholarships sc where sc.school_id=s.id and sc.status='active' and sc.verification_status='verified')::text scholarships from schools s where s.slug=$1", ["wuhan-university"]);
    if (counts[0]?.programs !== "0" || counts[0]?.scholarships !== "6") throw new Error(`WHU publication count mismatch: ${JSON.stringify(counts[0])}`);
    return { written, activeVerifiedScholarshipCount: 6, activeProgramCount: 0 };
  });
