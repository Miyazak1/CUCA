...
const paths = {
  candidate: resolve(root, "seeds/catalog.zju-complete-batch-01.draft.json"),
  validation: resolve(root, "seeds/catalog.zju-complete-batch-01.validation.json"),
  review: resolve(root, "seeds/catalog.zju-complete-batch-01.review.json"),
  prior: resolve(root, "seeds/catalog.zju-school-program-batch-01.approved.local.json"),
  output: resolve(root, "seeds/catalog.zju-complete-batch-01.approved.local.json"),
  approval: resolve(root, "seeds/catalog.zju-complete-batch-01.approval.json"),
...
if (storedReviewHash !== approvedReviewHash || actualReviewHash !== approvedReviewHash) throw new Error("Standing-authorized review hash does not match the stored ZJU review.");
const candidateFileSha256 = createHash("sha256").update(candidateText).digest("hex");
if (candidateFileSha256 !== review.candidateSha256) throw new Error("ZJU candidate changed after review.");
const freshValidation = createCatalogMigrationValidationReport(candidate);
if (!freshValidation.ok || !storedValidation.ok || freshValidation.bundleSha256 !== storedValidation.bundleSha256 || freshValidation.operationPlanSha256 !== storedValidation.operationPlanSha256) throw new Error("ZJU validation no longer matches the reviewed bundle.");
if (review.status !== "standing_user_approval" || review.standingAuthorization?.instruction !== "发布默认允许" || review.sensitiveDataCheck?.result !== "pass" || review.visualReview?.result !== "pass") throw new Error("ZJU review is not eligible for standing-authorized publication.");
if (review.scope?.newProgramRouteCount !== 588 || review.scope?.masterRouteCount !== 284 || review.scope?.doctoralRouteCount !== 304 || review.scope?.newProgramIntakeCount !== 588 || review.scope?.newScholarshipCount !== 2) throw new Error("ZJU review scope mismatch.");
...
if (JSON.stringify(candidate.cities) !== JSON.stringify(prior.cities) || JSON.stringify(candidate.schools) !== JSON.stringify(prior.schools)) throw new Error("ZJU dependency replay is not byte-equivalent to the prior approved bundle.");
...
const publication: CatalogSeedBundle = {
  ...
  handoff: { sourceSystem: "CUAC reviewed official Zhejiang University 2026 sources", cleanedExportName: "catalog.zju-complete-batch-01.draft.json", cleanedExportSha256: freshValidation.bundleSha256, sourceSchemaSha256, reviewReference, prohibitedDataReviewReference: reviewReference, approvalRecordedAt, sourceReadOnly: true, prohibitedDataConfirmedExcluded: true },
...
if (!publicationReport.ok || publicationReport.summary.schools !== 1 || publicationReport.summary.programs !== 588 || publicationReport.summary.programIntakes !== 588 || publicationReport.summary.scholarships !== 2) throw new Error(`ZJU publication bundle is invalid: ${publicationReport.errors.join(" ")}`);
const approvalRecord = { version: 1, approvedAt: approvalRecordedAt, reviewReference, approvedReviewSha256: approvedReviewHash, approvalMode: "standing-user-default-publication", approvedCandidate: "seeds/catalog.zju-complete-batch-01.draft.json", approvedCandidateFileSha256: candidateFileSha256, approvedCandidateBundleSha256: freshValidation.bundleSha256, publicationBundle: "seeds/catalog.zju-complete-batch-01.approved.local.json", publicationBundleSha256: publicationReport.bundleSha256, schoolSlug: "zhejiang-university", dependencyCitySlugs: candidate.cities?.map(row => row.slug), dependencySchoolReplay: true, programSlugs: publication.programs?.map(row => row.slug), scholarshipSlugs: publication.scholarships?.map(row => row.slug), archivedProgramAliases: [], archivedScholarshipAliases: [], prohibitedDataConfirmedExcluded: true };
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:zju-complete-publish" });
try {
  ...
  const result = await client.transaction(async tx => {
    ...
    const school = await tx.query<{ id: string }>("select id from schools where slug=$1 and status='active' and verification_status='verified'", ["zhejiang-university"]);
    if (school.length !== 1 || school[0].id !== "4339e060-8daf-43be-af9a-bb2f60237ce0") throw new Error(`ZJU dependency school does not match the reviewed target: ${JSON.stringify(school)}`);
    ...
    if (before.length !== 1 || before[0].degree_level !== "Undergraduate" || before[0].routes !== "92") throw new Error(`ZJU program baseline changed after review: ${JSON.stringify(before)}`);
    ...
    if (relations[0]?.intakes !== "98" || relations[0]?.scholarships !== "6") throw new Error(`ZJU related-record baseline changed after review: ${JSON.stringify(relations[0])}`);
    ...
    if (programConflicts.length || scholarshipConflicts.length) throw new Error(`ZJU publication conflicts appeared after review: ${JSON.stringify({ programConflicts, scholarshipConflicts })}`);
    ...
    if (counts[0]?.undergraduate !== "92" || counts[0]?.master !== "284" || counts[0]?.doctoral !== "304" || counts[0]?.intakes !== "686" || counts[0]?.scholarships !== "8" || counts[0]?.verified_scholarships !== "3") throw new Error(`ZJU post-publication count mismatch: ${JSON.stringify(counts[0])}`);
