...
const outputPath = resolve(root, "seeds/catalog.tongji-school-program-batch-01.approved.local.json");
const approvalPath = resolve(root, "seeds/catalog.tongji-school-program-batch-01.approval.json");
...
if (!freshValidation.ok || freshValidation.bundleSha256 !== approvedHash) throw new Error("Approved hash does not match a valid Tongji candidate bundle.");
...
if (review.status !== "awaiting_user_approval" || review.candidateBundleSha256 !== approvedHash) throw new Error("Tongji review is not awaiting approval for this candidate hash.");
if (review.sensitiveDataCheck?.result !== "pass" || review.sensitiveDataCheck?.scope !== "candidate bundle only") throw new Error("Tongji candidate has not passed prohibited-data review.");
if (review.scope?.schoolCount !== 1 || review.scope?.programCount !== 71 || review.scope?.intakeCount !== 71 || review.scope?.chineseProgramCount !== 68 || review.scope?.englishProgramCount !== 3) {
  throw new Error("Tongji review scope must contain exactly one school, 71 programs and 71 intakes.");
}
const legacyAliases = review.reconciliation?.legacyAliasesToArchive;
if (!Array.isArray(legacyAliases) || legacyAliases.length !== 0) throw new Error("Tongji reconciliation must not contain legacy aliases.");
const stableMatchingSlugs = review.reconciliation?.stableMatchingSlugs;
if (!Array.isArray(stableMatchingSlugs) || stableMatchingSlugs.length !== 13 || new Set(stableMatchingSlugs).size !== 13) {
  throw new Error("Tongji reconciliation must contain thirteen distinct stable matching slugs.");
}
if (stableMatchingSlugs.some(slug => !(candidate.programs ?? []).some(program => program.slug === slug))) {
  throw new Error("A reviewed stable matching slug is absent from the approved candidate.");
}
...
const publication: CatalogSeedBundle = {
  ...
  handoff: {
    sourceSystem: "CUAC reviewed official Tongji University 2026 web, PDF and XLS sources",
    cleanedExportName: "catalog.tongji-school-program-batch-01.draft.json",
...
if (!publicationReport.ok || publicationReport.summary.schools !== 1 || publicationReport.summary.programs !== 71 || publicationReport.summary.programIntakes !== 71) {
...
const approvalRecord = {
  ...
  reviewReference,
  approvedCandidate: "seeds/catalog.tongji-school-program-batch-01.draft.json",
  approvedCandidateBundleSha256: approvedHash,
  publicationBundle: "seeds/catalog.tongji-school-program-batch-01.approved.local.json",
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:tongji-school-program-publish" });
try {
  ...
  const result = await client.transaction(async tx => {
    ...
    const stableRows = await tx.query<{ slug: string; status: string; verification_status: string }>(
      ...
      ["tongji-university", stableMatchingSlugs],
    );
    if (stableRows.length !== stableMatchingSlugs.length || stableRows.some(row => row.status !== "active" || row.verification_status !== "unverified")) {
      throw new Error("The thirteen stable matches no longer exactly match the reviewed active/unverified state.");
    }
