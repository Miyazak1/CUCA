...
const paths = {
  candidate: resolve(root, "seeds/catalog.seu-complete-batch-02.draft.json"),
  validation: resolve(root, "seeds/catalog.seu-complete-batch-02.validation.json"),
  review: resolve(root, "seeds/catalog.seu-complete-batch-02.review.json"),
  output: resolve(root, "seeds/catalog.seu-complete-batch-02.approved.local.json"),
  approval: resolve(root, "seeds/catalog.seu-complete-batch-02.approval.json"),
...
if (storedReviewHash !== approvedReviewHash || actualReviewHash !== approvedReviewHash) throw new Error("Standing-authorized review hash does not match the stored SEU review.");
if (createHash("sha256").update(candidateText).digest("hex") !== review.candidateSha256) throw new Error("SEU candidate changed after review.");
const freshValidation = createCatalogMigrationValidationReport(candidate);
if (!freshValidation.ok || !storedValidation.ok || freshValidation.bundleSha256 !== storedValidation.bundleSha256 || freshValidation.operationPlanSha256 !== storedValidation.operationPlanSha256) throw new Error("SEU candidate validation no longer matches the reviewed bundle.");
if (review.status !== "standing_user_approval" || review.standingAuthorization?.instruction !== "发布默认允许" || review.sensitiveDataCheck?.result !== "pass") throw new Error("SEU review is not eligible for standing-authorized publication.");
if (review.scope?.schoolSlug !== "southeast-university" || review.scope?.schoolCount !== 1 || review.scope?.programRouteCount !== 232 || review.scope?.scholarshipCount !== 3) throw new Error("SEU review scope mismatch.");
...
const publication: CatalogSeedBundle = {
  ...
  handoff: {
    sourceSystem: "CUAC reviewed official Southeast University 2026 sources",
    cleanedExportName: "catalog.seu-complete-batch-02.draft.json",
...
if (!publicationReport.ok || publicationReport.summary.schools !== 1 || publicationReport.summary.programs !== 232 || publicationReport.summary.programIntakes !== 232 || publicationReport.summary.scholarships !== 3) throw new Error(`SEU publication bundle is invalid: ${publicationReport.errors.join(" ")}`);
const approvalRecord = {
  version: 1, approvedAt: approvalRecordedAt, reviewReference, approvedReviewSha256: approvedReviewHash,
  approvalMode: "standing-user-default-publication", approvedCandidate: "seeds/catalog.seu-complete-batch-02.draft.json",
  approvedCandidateBundleSha256: freshValidation.bundleSha256, publicationBundle: "seeds/catalog.seu-complete-batch-02.approved.local.json",
  publicationBundleSha256: publicationReport.bundleSha256, schoolSlug: "southeast-university",
...
const pool = createPostgresPool({ databaseUrl: target.databaseUrl, max: 1, applicationName: "cuac:seu-complete-publish" });
try {
  ...
  const result = await client.transaction(async tx => {
    ...
    const before = await tx.query<{ id: string }>("select id from schools where slug=$1 or lower(name_en)=lower($2) or name_zh=$3", ["southeast-university", "Southeast University", "东南大学"]);
    if (before.length !== 0) throw new Error("SEU record now exists; standing authorization requires a new conflict review before overwrite.");
    const city = await tx.query<{ name_en: string; name_zh: string; region: string }>("select name_en,name_zh,region from cities where slug=$1", ["nanjing"]);
    if (city.length !== 1 || city[0].name_en !== "Nanjing" || city[0].name_zh !== "南京") throw new Error(`Existing Nanjing city record is not the reviewed reference target: ${JSON.stringify(city)}`);
