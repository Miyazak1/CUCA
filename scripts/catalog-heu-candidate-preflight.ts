...
const candidate = JSON.parse(await readFile("seeds/catalog.heu-complete-batch-01.draft.json", "utf8"));
const state = JSON.parse(await readFile(".cuac-local/runtime.json", "utf8"));
const pool = createPostgresPool({ databaseUrl: localDatabaseUrl(state), max: 1, applicationName: "cuac:heu-candidate-preflight" });
try {
  ...
  const schoolPrograms = await pool.query("select id,slug,name_en,status,verification_status,source_url from programs where school_id=(select id from schools where slug='harbin-engineering-university') order by slug");
  ...
  const oldScholarships = await pool.query("select id,slug,title,status,verification_status,source_url from scholarships where school_id=(select id from schools where slug='harbin-engineering-university') order by slug");
